...
5. Paste this URL in the first box and leave the second box empty. Finally select a location on your HD where the repostiry will be cloned to.
...
# Tracking your file changes with `git` -- Basic workflow
...
We can make an anlology with taking a family picture, where eache family member would represent a file.

- **Add**ing files (to the stagging area), is like deciding which family member(s) are going to be on your next picture
- **Commit**ting is like taking the picture

These 2-step process enables you to flexibly group files into a specific commit.

These steps are repeated for every version you want to keep (every time you would like to use `save as`). Every time you commit, you create a new snapshot, you add the new version of the file to the git database, while keeping all the previous versions in the database. It creates an history of the content of your repostory that is like a graph that you can navigate:
