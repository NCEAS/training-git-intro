...
**GitHub is a company that hosts git repositories online** and provides several collaboration features (including `forking`). GitHub fosters a great user community and has built a nice web interface to git, also adding great visualization/rendering capacities of your data.
...
5. Paste this URL in the first box and do not change the second box (it will either remain blank or autofill with the name of the repository from the link, either is fine). Finally select a location on your hard drive ("HD") where the repository will be cloned to.
